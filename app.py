...
def save_qualifying_loans(qualifying_loans):
    ...
    if len(qualifying_loans) == 0:
        ...
        request = questionary.select("Would you like to save your loans in a new .csv file?", choices=["yes", "no"]).ask()
        if request == "no":
            sys.exit(f"Great, have a nice day!")
        else:
            if request == 'yes':
